...
async function deliver(
  ...
  const update: {
    "delivery.attempts": FirebaseFirestore.FieldValue,
    "delivery.endTime": FirebaseFirestore.Timestamp | FirebaseFirestore.FieldValue,
    "delivery.error"?: string | null,
    "delivery.leaseExpireTime": FirebaseFirestore.Timestamp | null,
    "delivery.state"?: "PENDING" | "PROCESSING" | "RETRY" | "SUCCESS" | "ERROR",
    "delivery.info"?: {
      messageId: string;
      accepted: string[];
      rejected: string[];
...
async function processWrite(change: any) { // Change<FirebaseFirestore.DocumentSnapshot>
...
export const processQueue = functions.firestore
  ...
  .onWrite(async (change) => {
    ...
    return await events.recordCompleteEvent(change)
      .then(() => logs.complete());
  });
