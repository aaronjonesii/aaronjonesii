...
gcloud config set project "$GCP_PROJECT" || {
  echo "Failed to set Google Cloud Project to $GCP_PROJECT"
  exit 1
}
...
gcloud run deploy "$GCP_SERVICE_NAME" --image "gcr.io/$GCP_PROJECT/$GCP_SERVICE_NAME" --region "$GCP_REGION_NAME" || {
  echo "Failed to deploy container image"
  exit 1
}

# Set Firebase project
firebase use "$FIREBASE_PROJECT" || {
  echo "Failed to set Firebase project to $FIREBASE_PROJECT"
  exit 1
}

# Deploy Firebase Hosting configuration
firebase deploy --only hosting -m "$DEPLOYMENT_MESSAGE" || {
  echo "Failed to deploy Firebase Hosting"
  exit 1
}
