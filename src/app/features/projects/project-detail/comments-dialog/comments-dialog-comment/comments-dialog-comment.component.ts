...
export class CommentsDialogCommentComponent {
  ...
  async onLikeComment() {
    try {
      ...
      const commentUpdates = {
        likes: this.comment?.likes?.some((ref) => ref.id === userRef.id) ?
          arrayRemove(userRef) : arrayUnion(userRef),
        dislikes: arrayRemove(userRef),
        updated: this.db.timestamp,
      };
      await this.db.batch(async (batch) => {
        /** fixme: investigate why this is needed here when used above */
        this._assertComment(this.comment);

        const commentRef = this.db.doc(
          `${this.comment.parent.path}/comments/${this.comment.id}`,
        );
        batch.update(commentRef, commentUpdates);
      }).catch((error) => {
        this.logger.error(
          `Something went wrong liking comment`,
          error, this.comment, this.user,
        );
      });
    } catch (error) {
      this.logger.error(
  ...
  async onDislikeComment() {
    try {
      ...
      const userRef = this.db.doc<UserWithID>(`users/${this.user?.id}`);
      /** check if user already dislikes the comment */
      ...
      await this.db.batch(async (batch) => {
        ...
        this.logger.error(
          `Something went wrong disliking comment`,
          error, this.comment, this.user,
        );
      });
    } catch (error) {
  ...
  get likesComment(): boolean {
    return this.comment?.likes
      ?.some((userRef) => userRef.id === this.user?.id) ?? false;
  }

  get dislikesComment(): boolean {
    if (!this.user || !this.comment) return false;
    return this.comment?.dislikes
      ?.some((userRef) => userRef.id === this.user?.id) ?? false;
  }
  ...
  async onReport(): Promise<void> {
    try {
      ...
      const userRef = this.db.doc<UserWithID>(`users/${this.user.id}`);
      const commentRef = this.db.doc<CommentWithID>(
        `${this.comment.parent.path}/comments/${this.comment.id}`,
      );
      ...
      return await this.db.batch(async (batch) => {
        ...
        this.logger.log(
          `Submitted report, thank you for helping our community.`,
        );
      })
        .catch((error) => {
          this.logger.error(
            `Something went wrong reporting comment`, error, report,
          );
        });
    } catch (error) {
      this.logger.error(
  ...
  get reported(): boolean {
    ...
    return this.user?.reported
      ?.some((report) => report.id == this.comment?.id) ?? false;
  }
